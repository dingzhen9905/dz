...
class SimpleCalendar {
	...
	addData(year, month) {
		...
		var bday = year +"-"+month+"-01 00:00:00";
        var enddate = year + "-" + month + "-" + lastday + " 23:59:59";
		var dataArr =["1","1","1"];
		// 查询数据
		searchfun("VR_find_vdhr421_01",bday,enddate,resData,0);
		searchfun("VR_find_vdhr422_425",bday,enddate,resData,1);
		searchfun("VR_find_vdac401_02",bday,enddate,resData,2);
...
//						}
						if(data[m][k]["日期日期"].slice(0, 4) == writeyear && data[m][k]["日期日期"].slice(5, 7) == monnum && data[m][k]["日期日期"].slice(8, 10) == daynum) {
							...
							if(m != 2){
								...
								daysElement[i].querySelector('.banci').innerHTML += data[m][k]["内容内容"];
							}
						}
					}	
				}
